    ...
    public static Pair<Integer, String> runCompiler(List<String> sourceFiles, String outputName, Boolean cpp, Project project) {
        ...
        try {
            ...
            if (exitCode == 0) {
                ...
                String errorMessage = getExitCodeDescription(exitCode);
                ret.append("Compilation failed with exit code: ").append(exitCode)
        ...
        try {
            ...
            if (SystemInfo.isWindows) {
                ...
                if (exitCode == 0) {
                    ...
                    String errorMessage = getExitCodeDescription(exitCode);
                    consoleWriteError("Program finished with exit code: " + exitCode + (errorMessage != null ? " (" + errorMessage + ")" : "") + "\n", project);
                }
            } else {
